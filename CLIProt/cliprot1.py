...
class CLIProt(nn.Module):
    ...
    def forward(self, protein_features):
        ...
        hs = torch.einsum("LBD->BLD", hs)
        return self.decoder(hs)
        # return hs_ppi_feature, hs_seq
        protein_features = torch.cat([hs_ppi_feature, hs_seq], dim=0)
        protein_features = torch.einsum("LBD->BLD", protein_features)
        # if self.decoder:
