...
class expert6(nn.Module):  # for 6x
    def __init__(self, input_dim, number, args):
        super().__init__()
    ...
    def forward(self, input):  # [6,32,512]
        x = input[self.number]  # 32,512
        out = self.decoder(x)
        return x, out


class expert0(nn.Module):  # for x1
    def __init__(self, input_num, input_dim, args):
        super().__init__()
...
class Predictor(nn.Module):
    def __init__(
        ...
        self.gate = GateNet(dim_feedforward * 6, 10, hard=True)
        # self.fc_decoder = FC_Decoder(
        #     num_class, dim_feedforward, activation, dropout, input_num=1
        # )
        self.expert0 = expert0(3, dim_feedforward, args)
        self.expert1 = expert1(3, dim_feedforward, args)
        self.expert2 = expert2(6, dim_feedforward, args)
        self.expert3 = expert3(6, dim_feedforward, args)
        self.expert_list = nn.ModuleList(
            [self.expert0, self.expert1, self.expert2, self.expert3]
        )

        for number in range(6):
            self.expert_list.append(expert6(dim_feedforward, number, args))
    ...
    def forward(self, src):
        ...
        weight = self.gate(fusion_hs_flatten)  # 32,10

        # 计算每个专家的输出
        expert_outputs = []
        last_hiddens = []
        for i, expert in enumerate(self.expert_list):
            last_hidden, expert_output = expert(
                fusion_hs
            )  # expert_output 形状为 [32, 512]
            expert_outputs.append(expert_output.unsqueeze(0))  # 1, 32, 45
            last_hiddens.append(last_hidden.unsqueeze(0))  # 1, 32, 512

        # 将所有专家输出堆叠在一起，形状变为 [4, 32, 512]
        expert_outputs = torch.cat(expert_outputs, dim=0)  # 10, 32, 45
        last_hiddens = torch.cat(last_hiddens, dim=0)  # 10, 32, 512
        ...
        selected_expert_hidden = torch.einsum(
            "bi,ibd->bd", weight, last_hiddens
        )  # 32, 512
