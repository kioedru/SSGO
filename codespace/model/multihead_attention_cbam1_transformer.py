...
from codespace.model.cbam_1 import CBAM
import torch
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.cbam = CBAM(dim_feedforward)
    ...
    def forward_post(
        ...
        src1 = torch.einsum("LBD->BDL", src)  # 32,512,3
        src1 = self.cbam(src1)
        src1 = torch.einsum("BDL->LBD", src1)  # 3,32,512
        src2, corr = self.self_attn(
            q, k, value=src1, attn_mask=src_mask, key_padding_mask=src_key_padding_mask
        )  # 此处的batch_first=false，因此输入是(L,B,D)

        src = src + self.dropout1(src2)  # 跳跃连接最开始的（在cbam之前）
        src = self.norm1(src)
    ...
    def forward(
        ...
        if self.normalize_before:  # 在之前是否做过归一化，一直使用的是forward_post
