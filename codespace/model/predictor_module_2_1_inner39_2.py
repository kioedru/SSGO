...
class FusionNet(nn.Module):
    def __init__(self, args, dim_feedforward):
        ...
        self.gate = GateNet(dim_feedforward * 5, 5, hard=False)

    def forward(self, hs_ppi_feature, hs_seq, hs_fc):

        fusion_hs = torch.cat(
            [hs_ppi_feature, hs_seq, hs_fc[0].unsqueeze(0), hs_fc[1].unsqueeze(0)],
            dim=0,
        )  # 5,32,512
        fusion_hs_flatten = torch.einsum("LBD->BLD", fusion_hs).flatten(1)  # 32,512*5

        weight = self.gate(fusion_hs_flatten)  # 32,5
        ppi_vec1 = weight[:, 0:1] * fusion_hs[0]  # 32,512
        feature_vec1 = weight[:, 1:2] * fusion_hs[1]  # 32,512
        seq_vec1 = weight[:, 2:3] * fusion_hs[2]  # 32,512
        ppi_vec2 = weight[:, 3:4] * fusion_hs[3]  # 32,512
        feature_vec2 = weight[:, 4:5] * fusion_hs[4]  # 32,512
        gate_hs = torch.stack(
            [ppi_vec1, feature_vec1, seq_vec1, ppi_vec2, feature_vec2], dim=0
        )  # 5,32,512
