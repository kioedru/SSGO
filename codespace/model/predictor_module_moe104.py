...
class Predictor(nn.Module):
    def __init__(
        ...
        self.fusion_model17 = Fusion_Model17(args)

        self.fc_decoder_for_model17 = FC_Decoder(
            num_class, dim_feedforward, activation, dropout, input_num=3
        )

        self.fc_decoder_for_model39 = FC_Decoder(
            num_class, dim_feedforward, activation, dropout, input_num=3
        )
    ...
    def forward_for_model17(self, src):
        ...
        hs_model17 = self.fusion_model17(hs_model17)  # 3,32,512
        output_model17 = self.fc_decoder_for_model17(hs_model17)  # 32,45
        return output_model17, hs_model17
    ...
    def forward_for_model39(self, src):
        ...
        output_model39 = self.fc_decoder_for_model39(hs_model39)
        return output_model39, hs_model39

    def forward(self, src):
        output_model17, hs_model17 = self.forward_for_model17(src)
        output_model39, hs_model39 = self.forward_for_model39(src)
        expert_outputs = torch.stack([output_model17, output_model39], dim=0)  # 2,32,45
        expert_outputs_flatten = torch.einsum("LBD->BLD", expert_outputs).flatten(
            1
        )  # 32,45*2
        weight = self.gate(expert_outputs_flatten)  # 32,2

        # 使用 torch.einsum 正确地计算加权专家输出
        # weight的形状是 [32, 2]，expert_outputs的形状是 [2, 32, 45]，希望输出为 [32, 45]
