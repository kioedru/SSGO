...
class CorNet(nn.Module):
    def __init__(self, output_size, cornet_dim=100, n_cornet_blocks=2, **kwargs):
        ...
        for layer in self.intlv_layers:  # 初始化权重
...
if __name__ == "__main__":
    ...
    model = CorNet(
        output_size=output_size, cornet_dim=cornet_dim, n_cornet_blocks=n_cornet_blocks
    )

    input_tensor = torch.rand(32, 4, output_size)
