...
class Predictor(nn.Module):
    def __init__(
        ...
        self.seq_pre_model17_adapter = mlp_adapter(
            dim_feedforward, dim_feedforward * 2, dropout
        )
        self.ppi_feature_pre_model17_adapter = mlp_adapter(
            dim_feedforward, dim_feedforward * 2, dropout
        )
    ...
    def forward(self, src):
        ...
        for num in range(self.attention_layers_num):
            ...
            ppi_feature_output = (
                self.ppi_feature_pre_model.transformerEncoder.encoder.layers[num](
                    ppi_feature_output, seq_encoder_output
                )
            )
            ...
            seq_output = self.seq_pre_model.transformerEncoder.encoder.layers[num](
                seq_output, ppi_feature_encoder_output
            )
        ...
        seq_encoder_output = seq_fc  # 1,32,512
        for num in range(self.attention_layers_num):
            ppi_feature_output = self.ppi_feature_pre_model17_adapter(
                ppi_feature_encoder_output
            )  # 2,32,512
            ppi_feature_output = (
            ...
            seq_output = self.seq_pre_model17_adapter(seq_encoder_output)  # 1,32,512
        ...
        fusion_model39 = self.fusion_model39(
            ppi_feature_output, seq_output, hs_model17
        )  # 32,45

        fusion_hs = torch.stack([fusion_model17, fusion_model39], dim=0)  # 2,32,45
