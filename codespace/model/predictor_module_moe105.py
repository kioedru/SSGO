...
class expert_C62(nn.Module):  # for 6x
    def __init__(self, input_num, input_dim, number1, number2, args):
        super().__init__()
    ...
    def forward(self, input):  # [6,32,512]
        x1 = input[self.number1]  # 32,512
        x2 = input[self.number2]  # 32,512
        x = torch.cat([x1, x2], dim=1)  # 32,512*2
        x = self.fc1(x)  # 32,512
        x = self.relu(x)
        x = self.fc2(x)  # 32,512
        return x
...
class expert6(nn.Module):  # for 6x
    def __init__(self, input_num, input_dim, number, args):
        super().__init__()
        self.fc1 = nn.Linear(input_dim * input_num, input_dim)
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(input_dim, input_dim)
...
class Predictor(nn.Module):
    def __init__(
        ...
        self.gate = GateNet(dim_feedforward * 6, 10 + 15, hard=True)
        ...
        for number in range(6):
            self.expert_list.append(expert6(1, dim_feedforward, number, args))

        # C62
        combinations = list(itertools.combinations(range(6), 2))
        for number1, number2 in combinations:
            self.expert_list.append(
                expert_C62(2, dim_feedforward, number1, number2, args)
            )
    ...
    def forward(self, src):
        ...
        weight = self.gate(fusion_hs_flatten)  # 32,25
        ...
        # 将所有专家输出堆叠在一起，形状变为 [25, 32, 512]
        expert_outputs = torch.cat(expert_outputs, dim=0)  # 25, 32, 512
