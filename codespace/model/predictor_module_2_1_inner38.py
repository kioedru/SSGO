...
class FusionNet(nn.Module):
    def __init__(self, args, dim_feedforward):
        ...
        self.gate = GateNet(dim_feedforward * 3, 3, hard=True)
    ...
    def forward(self, hs_ppi_feature, hs_seq):
        ...
        fusion_hs_flatten = torch.einsum("LBD->BLD", fusion_hs).flatten(1)  # 32,1536

        weight = self.gate(fusion_hs_flatten)  # 32,3
        ppi_vec1 = weight[:, 0:1] * fusion_hs[0]  # 32,512
        feature_vec1 = weight[:, 1:2] * fusion_hs[1]  # 32,512
        seq_vec1 = weight[:, 2:3] * fusion_hs[2]  # 32,512
        gate_hs = torch.stack([ppi_vec1, feature_vec1, seq_vec1], dim=0)  # 3,32,512
