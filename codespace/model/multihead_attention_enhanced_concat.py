...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        # bimamba，设置单词维度为1
        self.self_attn = EnhancedSemanticAttentionModule(
            global_dim=dim_feedforward, local_dim=dim_feedforward * 2, num_heads=nhead
        )
    ...
    def forward_post(
        ...
        src2 = self.self_attn(seq_src, ppi_feature_src)  # [32,1,1536]
        src2 = src2.reshape(-1, 1, L, D)
        # 调整维度顺序，得到形状为 [3, 32, 512] 的张量
        src2 = src2.permute(2, 0, 3, 1).reshape(L, -1, D)  # [3,32,512]

        src = src + self.dropout1(src2)  # [3,32,512]
        src = self.norm1(src)
