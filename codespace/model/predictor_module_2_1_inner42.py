...
class FusionNet(nn.Module):
    def __init__(self, args, dim_feedforward):
        ...
        self.gate = GateNet(dim_feedforward * 7, 7, hard=False)
    ...
    def forward(self, hs_ppi_feature, hs_seq, hs_model17):
        ...
        fusion_hs = torch.cat(
            [hs_ppi_feature, hs_seq, model17_fusion], dim=0
        )  # 7,32,512
        fusion_hs_flatten = torch.einsum("LBD->BLD", fusion_hs).flatten(1)  # 32,512*7
        fusion_hs_permuted = torch.einsum("LBD->BLD", fusion_hs)  # 32, 7, 512
        weight = self.gate(fusion_hs_flatten).unsqueeze(-1)  # 32,7,1
        weighted_gate_hs = fusion_hs_permuted * weight  # 32, 7, 512
        weighted_gate_hs = torch.einsum("BLD->LBD", weighted_gate_hs)  # 7, 32, 512

        return weighted_gate_hs
