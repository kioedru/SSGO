...
from codespace.model.cornet import CorNet
...
class Fusion_Model17(nn.Module):
    def __init__(self, args, dim_feedforward):
        ...
        self.cornet = CorNet(dim_feedforward, dim_feedforward * 2, 2)
    ...
    def forward(self, hs_ppi_feature, hs_seq, hs_model17):
        ...
        model17_fusion = torch.einsum("LBD->BLD", model17_fusion)  # 32,4,512
        model17_fusion = self.cornet(model17_fusion)  # 32,4,512
        model17_fusion = torch.einsum("BLD->LBD", model17_fusion)  # 4,32,512
...
class Fusion_Model39(nn.Module):
    ...
    def forward(self, hs_ppi_feature, hs_seq, hs_model17):
        ...
        model39_fusion = self.cornet(weighted_gate_hs)  # 32,3,512
        model39_fusion = torch.einsum("BLD->LBD", model39_fusion)  # 3,32,512
        model39_fusion = self.decoder_model39(model39_fusion)  # 32,45
