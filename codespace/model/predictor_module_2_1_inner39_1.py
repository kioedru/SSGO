...
class FusionNet(nn.Module):
    def __init__(self, args, dim_feedforward):
        ...
        self.gate = GateNet(dim_feedforward * 4, 4, hard=False)

    def forward(self, hs_ppi_feature, hs_seq, hs_fc):

        fusion_hs = torch.cat(
            [hs_ppi_feature, hs_seq, hs_fc[0].unsqueeze(0)], dim=0
        )  # 4,32,512
        fusion_hs_flatten = torch.einsum("LBD->BLD", fusion_hs).flatten(1)  # 32,512*4
        ...
        gate_hs = torch.stack(
            [ppi_vec1, feature_vec1, seq_vec1, ppi_vec2], dim=0
        )  # 4,32,512
