...
class Fusion(nn.Module):
    ...
    def forward(self, hs_ppi_feature, hs_seq):
        ...
        model_fusion = self.decoder_model(weighted_gate_hs)  # 32,45
        return model_fusion
...
class Predictor(nn.Module):
    def __init__(
        ...
        self.seq_pre_model_adapter = nn.ModuleList(
            [
                copy.deepcopy(
                    mlp_adapter(dim_feedforward, dim_feedforward * 2, dropout)
                )
                for i in range(self.attention_layers_num)
            ]
        )
        self.ppi_feature_pre_model_adapter = nn.ModuleList(
            [
                copy.deepcopy(
                    mlp_adapter(dim_feedforward, dim_feedforward * 2, dropout)
                )
        ...
        # 交叉注意力
        self.ppi_feature_crs_attn = nn.ModuleList(
            [
                copy.deepcopy(
                    nn.MultiheadAttention(dim_feedforward, args.nheads, dropout=dropout)
                )
                for i in range(self.attention_layers_num)
            ]
        )
        self.seq_crs_attn = nn.ModuleList(
            [
                copy.deepcopy(
                    nn.MultiheadAttention(dim_feedforward, args.nheads, dropout=dropout)
                )
        ...
        self.gate = GateNet(num_class * 2, 2, hard=True)
        self.fusion_model = Fusion(args, dim_feedforward, input_num)
    ...
    def forward(self, src):
        ...
        for num in range(self.attention_layers_num):
            ppi_feature_output = self.ppi_feature_pre_model_adapter[num](
                ppi_feature_encoder_output
            )  # 2,32,512
            seq_output = self.seq_pre_model_adapter[num](seq_encoder_output)  # 1,32,512

            seq_output_for_crs_attn = torch.cat(
                [seq_output, seq_output], dim=0
            )  # 2,32,512

            # 微调的交叉注意力
            ppi_feature_output, _ = self.ppi_feature_crs_attn[num](
                ppi_feature_output, seq_output_for_crs_attn, seq_output_for_crs_attn
            )  # 2,32,512
            seq_output, _ = self.seq_crs_attn[num](
                seq_output_for_crs_attn, ppi_feature_output, ppi_feature_output
            )  # 2,32,512
            seq_output = seq_output.mean(dim=0, keepdim=True)  # 1,32,512
