...
class pretrainLossOptimized(nn.Module):
    ...
    def forward(self, ori, rec, hs):
        ...
        rec_s = rec[2].squeeze()
        rec_t = rec[3].squeeze()
        recon_loss_1 = torch.mean(
            torch.sum(
                F.binary_cross_entropy_with_logits(rec_x, x, reduction="none"), dim=1
            )
        )
        recon_loss_2 = torch.mean(
            torch.sum(
                F.binary_cross_entropy_with_logits(rec_z, z, reduction="none"), dim=1
            )
        )
        recon_loss_3 = torch.mean(
            torch.sum(
                F.binary_cross_entropy_with_logits(rec_s, s, reduction="none"), dim=1
            )
        )
        recon_loss_4 = torch.mean(
            torch.sum(
                F.binary_cross_entropy_with_logits(rec_t, t, reduction="none"), dim=1
            )
        )
