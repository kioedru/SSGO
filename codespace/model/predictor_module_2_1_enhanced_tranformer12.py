...
class Predictor(nn.Module):
    ...
    def forward(self, src):
        ...
        global_seq = torch.cat([global_seq, global_seq], dim=1)  # 32,2,512
        local_seq = torch.einsum("LBD->BLD", seq_src)  # 32,1,512
        local_seq = torch.cat([local_seq, local_seq], dim=1)  # 32,2,512

        fusion_ppi_feature = self.fusion_ppi_feature(
            global_ppi_feature, local_seq
        )  # 32,2,1024
        ...
        fusion_seq = self.fusion_seq(global_seq, local_ppi_feature)  # 32,2,1024
        fusion_seq = torch.einsum("BLD->LBD", fusion_seq)  # 2,32,1024

        fusion_seq = self.fc_seq(fusion_seq)  # 2,32,512
        fusion_seq = self.act_seq(fusion_seq)  # 2,32,512
        fusion_seq = self.drop_seq(fusion_seq)  # 2,32,512
