...
from codespace.utils.MultiModal.QueryEnhancedSemanticModule import (
    QueryEnhancedSemanticModule,
)
...
class Predictor(nn.Module):
    def __init__(
        ...
        self.fusion_ppi_feature = QueryEnhancedSemanticModule(
            global_dim=dim_feedforward, local_dim=dim_feedforward, num_heads=8
        )
        self.fusion_seq = QueryEnhancedSemanticModule(
            global_dim=dim_feedforward, local_dim=dim_feedforward, num_heads=8
        )
    ...
    def forward(self, src):
        ...
        for num in range(self.attention_layers_num):
            ppi_feature_output = self.fusion_ppi_feature(
                ppi_feature_encoder_output,
                torch.cat([seq_encoder_output, seq_encoder_output], dim=0),
            )  # 2,32,1024
            ppi_feature_output = self.fc_ppi_feature(ppi_feature_output)  # 2,32,512
            seq_encoder_output = self.fusion_seq(
                torch.cat([seq_encoder_output, seq_encoder_output], dim=0),
                ppi_feature_encoder_output,
            )  # 2,32,1024
            seq_encoder_output = seq_encoder_output.mean(dim=0).unsqueeze(
                0
            )  # 1,32,1024
            seq_encoder_output = self.fc_seq(seq_encoder_output)  # 1,32,512
