...
class Predictor(nn.Module):
    def __init__(
        ...
        self.seq_pre_model_adapter = mlp_adapter(
            dim_feedforward, dim_feedforward * 2, dropout
        )
        self.ppi_feature_pre_model_adapter = mlp_adapter(
            dim_feedforward, dim_feedforward * 2, dropout
        )

        # 交叉注意力
        self.ppi_feature_crs_attn = nn.MultiheadAttention(
            dim_feedforward, args.nheads, dropout=dropout
        )
        self.seq_crs_attn = nn.MultiheadAttention(
            dim_feedforward, args.nheads, dropout=dropout
        )
    ...
    def forward(self, src):
        ...
        for num in range(self.attention_layers_num):
            ppi_feature_output = self.ppi_feature_pre_model_adapter(
                ppi_feature_encoder_output
            )  # 2,32,512
            seq_output = self.seq_pre_model_adapter(seq_encoder_output)  # 1,32,512
            ...
            # 微调的交叉注意力
            ppi_feature_output, _ = self.ppi_feature_crs_attn(
                ppi_feature_output, seq_output_for_crs_attn, seq_output_for_crs_attn
            )  # 2,32,512
            seq_output, _ = self.seq_crs_attn(
