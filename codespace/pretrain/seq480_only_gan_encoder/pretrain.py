...
def get_dataset(aspect, organism_num):
    ...
    # )
    train_seq, train_labels = read_gan_seq_embed_and_labels(
        aspect, organism_num, "esm2-480"
    )
    test_seq, test_labels = get_finetune_data("test", aspect, organism_num)

    combine_seq = train_seq
    combine_labels = train_labels
...
def main():
    ...
    args.model_name = f"seq480_only_gan_encoder"
    ...
    args.epoch_performance_path = os.path.join(
        ...
        args.org,
        args.aspect,
        f"epoch_performance_lr:{params['lr']}_dropout:{params['dropout']}.csv",
    )
