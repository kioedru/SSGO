...
def get_finetune_data(usefor, aspect, organism_num, feature_name):
    if feature_name == "ppi":
        feature = read_ppi_by_index(usefor, aspect, organism_num)
    elif feature_name == "seq480":
        feature = read_seq_embed_avgpool_esm2_480_by_index(usefor, aspect, organism_num)
    elif feature_name == "feature":
        feature = read_feature_by_index(usefor, aspect, organism_num)
    elif feature_name == "seq1024":
        feature = read_seq_embed_avgpool_prott5_1024_by_index(
...
def get_dataset(aspect, organism_num, feature_name):
    train_feature, train_labels = get_finetune_data(
        "train", aspect, organism_num, feature_name
    )
    valid_feature, valid_labels = get_finetune_data(
        "valid", aspect, organism_num, feature_name
    )
    test_feature, test_labels = get_finetune_data(
        "test", aspect, organism_num, feature_name
    )
    ...
    train_dataset = multimodesDataset(1, [combine_feature], combine_labels)
    test_dataset = multimodesDataset(1, [test_feature], test_labels)
...
def main():
    ...
    args.model_name = f"one_feature_only"
    args.model_one_feature_name = f"{args.backbone}_{args.feature_name}_only"
    ...
    args.pretrained_model = os.path.join(
        ...
        args.model_name,
        args.org,
        f"{args.model_one_feature_name}.pkl",
    )
...
def main_worker(args):
    ...
    train_dataset, test_dataset, args.modesfeature_len = get_dataset(
        args.aspect, args.org, args.feature_name
    )
