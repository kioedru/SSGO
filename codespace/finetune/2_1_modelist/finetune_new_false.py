...
# nohup python -u /home/Kioedru/code/SSGO/codespace/finetune/2_1_modelist/finetune.py --model_num 47 --seq_feature seq1024 --aspect P --num_class 45 --seed 1329765522 --device cuda:0 &
# nohup python -u /home/Kioedru/code/SSGO/codespace/finetune/2_1_modelist/finetune.py --model_num 47 --seq_feature seq1024 --aspect F --num_class 38 --seed 1329765522 --device cuda:0 &
# nohup python -u /home/Kioedru/code/SSGO/codespace/finetune/2_1_modelist/finetune.py --model_num 47 --seq_feature seq1024 --aspect C --num_class 35 --seed 1329765522 --device cuda:0 &
def main():
    args = get_args()
    ...
    # }
    params = {
        "lr": 0.021873915093390775,
        "dropout": 0.3386216107871549,
        "lr_model17": 0.00014303850645524388,
        "lr_model39": 0.000014852729879134244,
    }
    if args.param == 1:
        params = {
            "lr": args.lr,
            "dropout": args.dropout,
            "lr_model17": args.lr_model17,
            "lr_model39": args.lr_model39,
        }
    # 获取需要评估的超参
    ...
    args.org = "9606"
    args.model_name = f"2_1_modelist_{args.model_num}_{args.seq_feature}"
    ...
    args.epoch_performance_path = os.path.join(
        args.finetune_path,
        f"epoch_performance_lr:{params['lr']},dropout:{params['dropout']}.csv",
    )
    ...
    args.lr_model17 = params["lr_model17"]
    args.lr_model39 = params["lr_model39"]
...
def main_worker(args):
    ...
    if args.model_num == "45":
        from codespace.model.predictor_module_2_1_modelist45 import build_predictor
    elif args.model_num == "46":
        from codespace.model.predictor_module_2_1_modelist46 import build_predictor
    elif args.model_num == "47":
        from codespace.model.predictor_module_2_1_modelist47 import build_predictor
    ...
    print(
        f"lr:{args.lr}, lr_model17:{args.lr_model17}, lr_model39:{args.lr_model39},dropout:{args.dropout}"
    )
    predictor_model_param_dicts = [
        ...
        },
        {
            "params": [
                p
                for n, p in predictor_model.fusion_model39.named_parameters()
                if p.requires_grad
            ],
            "lr": args.lr_model39,
        },
        ...
        },
        {
            "params": [
                p
                for n, p in predictor_model.seq_pre_model17_adapter.named_parameters()
                if p.requires_grad
            ],
        },
        {
            "params": [
                p
                for n, p in predictor_model.ppi_feature_pre_model17_adapter.named_parameters()
                if p.requires_grad
            ],
        },
    ]
    # 初始化用于存储所有cross_attn参数的字典
    cross_attn_dict = {
        "params": [],
        # 使用默认学习率
    }

    # 遍历 Transformer 的每一层并提取 cross-attn 的参数
    for layer in predictor_model.seq_pre_model.transformerEncoder.encoder.layers:
        cross_attn_params = [
            p for p in layer.cross_attn.parameters() if p.requires_grad
        ]
        cross_attn_dict["params"].extend(
            cross_attn_params
        )  # 将参数追加到同一个字典的 "params" 列表中

    predictor_model_param_dicts.append(cross_attn_dict)

    if args.model_num == "45" or args.model_num == "46":
        predictor_model_param_dicts.append({"params": [predictor_model.alpha]})

    if args.model_num == "47":
        predictor_model_param_dicts.append(
            {
                "params": [
                    p
                    for n, p in predictor_model.gate.named_parameters()
                    if p.requires_grad
                ],
            }
        )
...
def finetune(
    ...
    for epoch in range(num_epochs):
        ...
        for p in net.seq_pre_model.parameters():
            p.requires_grad = False
        for p in net.ppi_feature_pre_model17.parameters():
            p.requires_grad = False
        for p in net.seq_pre_model17.parameters():
            p.requires_grad = False

        # 将新增加的层的参数设为可训练
        for layer in net.ppi_feature_pre_model.transformerEncoder.encoder.layers:
            for param in layer.cross_attn.parameters():
                param.requires_grad = True
        for layer in net.seq_pre_model.transformerEncoder.encoder.layers:
            for param in layer.cross_attn.parameters():
                param.requires_grad = True
        start = time.time()
        batch_count = 0
        ...
        # 每轮都打印一次学习率
        print(
            f"Epoch {epoch}, lr: {optimizer.param_groups[2]['lr']},  lr_model17: {optimizer.param_groups[0]['lr']}, lr_model39: {optimizer.param_groups[1]['lr']},dropout: {args.dropout}"
        )
