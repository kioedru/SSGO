...
def parser_args():
    ...
    parser.add_argument(
        "--seq_feature",
        default="seq1024",  # seq1024 seq480
        type=str,
    )
...
# nohup python -u /home/Kioedru/code/SSGO/codespace/finetune/2_1_simple/finetune_nofusion.py --fusion transformer --seq_feature seq1024 --aspect P --num_class 45 &
# nohup python -u /home/Kioedru/code/SSGO/codespace/finetune/2_1_simple/finetune_nofusion.py --fusion transformer --seq_feature seq1024 --aspect F --num_class 38 &
# nohup python -u /home/Kioedru/code/SSGO/codespace/finetune/2_1_simple/finetune_nofusion.py --fusion transformer --seq_feature seq1024 --aspect C --num_class 35 &
def main():
    args = get_args()
    ...
    # args.fusion = "transformer"  # transformer bimamba mamba enhanced
    # args.aspect = "P"
    ...
    args.model_name = f"2_1_{args.seq_feature}_simple_fusion={args.fusion}"
    ...
    args.epoch_performance_path = os.path.join(
        args.finetune_path,
        f"seed={args.seed}_2layer_epoch_performance_lr:{params['lr']},dropout:{params['dropout']},pre_lr:{params['pre_lr']},seq_pre_lr:{params['seq_pre_lr']}.csv",
    )
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if not args.nni:
                ...
                if args.aspect == "P":
                    if perf["m-aupr"] > 0.33 and perf["Fmax"] > 0.44:
                        torch.save(
                            net,
                            os.path.join(
                                args.finetune_model_path,
                                f"seed={args.seed}_epoch{epoch}_model_m-aupr{perf['m-aupr']}_Fmax{perf['Fmax']}.pkl",
                            ),
                        )
                if args.aspect == "F":
                    if perf["m-aupr"] > 0.16 and perf["Fmax"] > 0.245:
                        torch.save(
                            net,
                            os.path.join(
                                args.finetune_model_path,
                ...
                if args.aspect == "C":
                    if perf["m-aupr"] > 0.35 and perf["Fmax"] > 0.4:
                        torch.save(
                            net,
                            os.path.join(
                                args.finetune_model_path,
