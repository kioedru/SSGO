...
class EnhancedSemanticAttentionModule(nn.Module):
    ...
    def forward(self, global_features, local_features):
        ...
        global_to_local_attn, _ = self.global_to_local_attention(
            local_features, adjusted_global_features, adjusted_global_features
        )
        local_to_global_attn, _ = self.local_to_global_attention(
            global_features, adjusted_local_features, adjusted_local_features
        )

        # Concatenate the cross-attention outputs
        concatenated_features = torch.cat(
            (global_to_local_attn, local_to_global_attn), dim=2
        )

        # Self-attention to enhance the features further
        enhanced_features, _ = self.self_attention(
            concatenated_features, concatenated_features, concatenated_features
        )
...
if __name__ == "__main__":
    ...
    local_input = torch.rand(32, 1, 512).cuda()  # Example local features
    block = EnhancedSemanticAttentionModule(
        global_dim=512, local_dim=512, num_heads=8
    ).cuda()
