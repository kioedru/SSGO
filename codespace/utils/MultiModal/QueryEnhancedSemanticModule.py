...
class QueryEnhancedSemanticModule(nn.Module):
    def __init__(self, global_dim, local_dim, num_heads=8):
        ...
        self.global_feature_attention = CrossAttentionWithLearnableQuery(
            self.global_dim, num_heads
        )
        self.local_feature_attention = CrossAttentionWithLearnableQuery(
            self.local_dim, num_heads
        )

        # Cross-Attention for concatenated features
        self.concat_feature_attention = CrossAttentionWithLearnableQuery(
            self.global_dim + self.local_dim, num_heads
        )  # Adjusted dimension
    ...
    def forward(self, global_features, local_features):
        ...
        concatenated_features = torch.cat(
            (agg_global_features, agg_local_features), dim=2
        )
...
if __name__ == "__main__":
    ...
    global_input = torch.rand(2, 32, 512).cuda()  # L,B,D
    local_input = torch.rand(2, 32, 512).cuda()  # L,B,D
    block = QueryEnhancedSemanticModule(
